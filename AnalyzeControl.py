...
def init_cfg(cfg_path, args):
	...
	# adjustment
	file_md5 = base.BaseAnalyzer.get_md5_by_fname(cfg['target'])
	file_log_dir = os.path.join(BASE_HOME, cfg["log_dir"],file_md5)
...
def init_arguments(argv):
	...
	parser.add_argument('-a', '--args', help='Set target args', type=str, default='',
						required=False)
	# required arg
...
def is_executable(file_path):
	...
	exec_type_list = ['ELF']
	sh_type_list = ['POSIX','Bourne-Again']
	ret = 0
	if file_type in exec_type_list:
		if -1 != full_info.find("executable"):
			ret = 1
	elif file_type in sh_type_list:
		if -1 != full_info.find("executable"):
			ret = 2
...
def do_work(cfg, do_static, do_dynamic):
	...
	if do_dynamic:
		exe_type = is_executable(cfg.target)
		if exe_type == 1:
