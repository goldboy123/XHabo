...
function on_init()
	...
	filter = string.format("%s (proc.pid >= %d or proc.apid = %d or proc.name contains bash ) and", filter, g_pid, g_pid)
	filter = string.format("%s evt.type=write and evt.arg.data contains +", filter)
