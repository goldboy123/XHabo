...
void target_loader(char* target, char *args[], char** envp){
	int ret = execve(target,&args[1],envp);
	if (-1==ret){
...
void usage(){
	printf("usage: target_loader target_path [args]\n");
}
int main(int argc, char** argv, char** envp){
	...
	sig_manager();
	pause();
	target_loader(argv[1],argv,envp);
	return 0;
}
