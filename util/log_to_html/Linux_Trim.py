...
def classifyLogInfo(dynamicLog):
    ...
    infoDict = {}
    infoDict["Dynamic"] = {"Process":{}, "File":{}, "Net":{}, "Other":OrderedDict()}
    ...
    if os.path.exists(dynamicLog):
                ...
                if dyID in (8020005, 8020006, 8020007, 8020008, 8020009, 8020010, 8020011, 8020012):
                    ...
                    if len(item) >= 6:
                        ...
                        if comment.startswith(u'+'):
                            key = 'bash seq'
                            if infoDict["Dynamic"]["Other"].has_key(key):
                                infoDict["Dynamic"]["Other"][key].append(comment)
                            else:
                                infoDict["Dynamic"]["Other"][key] = list([comment])
        ...
        for key in infoDict["Dynamic"]["Other"]:
            list_tmp = infoDict["Dynamic"]["Other"][key]
            infoDict["Dynamic"]["Other"][key] = list(set(infoDict["Dynamic"]["Other"][key]))[0:15]
            infoDict["Dynamic"]["Other"][key].sort(key = list_tmp.index)
